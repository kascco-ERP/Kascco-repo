...
class PurchaseOrder(models.Model):
    ...
    qr_code = fields.Binary(string="QR Code", compute="_generate_qr_code_e_purchase")
    purchase_terms_conditions_ar = fields.Html(string="Terms and Conditions (AR)", track_visibility='onchange')
    purchase_terms_conditions_en = fields.Html(string="Terms and Conditions (EN)", track_visibility='onchange')

    @api.onchange('company_id')
    def _onchange_company_id(self):
        for rec in self:
            rec.purchase_terms_conditions_ar = rec.company_id.purchase_terms_conditions_ar
            rec.purchase_terms_conditions_en = rec.company_id.purchase_terms_conditions_en
    ...
    def get_qr_code_vals(self):
        ...
        vendor_name = str(self.partner_id.name)
        vendor_vat_no = str(self.partner_id.vat)
        enypt_vendor_name = self._get_hexa("01", "0c", vendor_name)
        enypt_vendor_vat_no = self._get_hexa("02", "0f", vendor_vat_no)
        ...
        enypt_qr = enypt_vendor_name + enypt_vendor_vat_no + enypt_date + enypt_amount_total_with_vat + enypt_amount_vat
        encoded_base64_bytes = base64.b64encode(bytes.fromhex(enypt_qr)).decode()
        return encoded_base64_bytes

    def _generate_qr_code_e_purchase(self):
